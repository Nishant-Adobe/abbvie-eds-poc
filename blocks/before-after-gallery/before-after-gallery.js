...
export default function decorate(block) {
  ...
  if (config.instructionText) { const instr = document.createElement('div'); instr.className = 'before-after-gallery-instruction'; instr.textContent = config.instructionText; content.append(instr); }
  ...
  if (config.tabs.length > 1) {
    const tabs = buildTabs(config, isToggle, renderTab);
    wrapper.append(tabs);
  }
