...
/**
 * Page-level parser for the RINVOQ HCP /rheumatoid-arthritis page.
 *
 * SELECTOR STRATEGY (documented assumption):
 * The live rinvoqhcp DOM exposes ambiguous repeated wrappers that cannot be
 * reliably disambiguated per-section by CSS selector alone. Per project memory
 * ("Live HTML is reference only"), this single page-level parser is keyed on
 * `body` and DETERMINISTICALLY composes every section in gold-standard order
 * using the verbatim content captured in
 * import-work-rinvoqhcp/rheumatoid-arthritis/analysis.md plus the regulated
 * copy reused VERBATIM from the approved homepage content/rinvoq-hcp/index.plain.html.
 *
 ...
 * any top-level `import` would throw a SyntaxError.
 *
 * Block-table shapes are modeled EXACTLY on the gold-standard plain.html:
 *   content/rinvoq-hcp/index.plain.html (brand-explorer, cards-grid-cta-card,
 *   text-container boxed-warning/legal/references, safety-bar split, metadata)
 *   content/rinvoq-hcp/dermatology/dosing-lab-monitoring.plain.html (hero no-padding
 *   8-row + section-metadata classes_customClass).
 *
 * Project rules honored:
 *   - No <sup>: literal Unicode (¹²³⁴⁵⁶⁷⁸⁹⁰ ‡ § || † *).
 *   - section custom classes via classes_customClass (text field).
 *   - Verbatim regulated copy (ISI, references) byte-identical to homepage.
 *
 * @param {Element} element The `body` element (its children are replaced).
...
export default function parse(element, { document }) {
  ...
  const heading = (level, html) => { const el = document.createElement(level); el.innerHTML = html; return el; };
  const anchor = (href, text) => { const a = document.createElement('a'); a.setAttribute('href', href); a.textContent = text; return a; };
  const div = (...nodes) => { const d = document.createElement('div'); nodes.forEach((n) => { if (n != null) d.append(n); }); return d; };
  const hr = () => document.createElement('hr');
  const table = (cells) => WebImporter.DOMUtils.createTable(cells, document);
  const fromHTML = (html) => { const d = document.createElement('div'); d.innerHTML = html; return d; };
  // Convert a hand-written `<div class="block">…</div>` (rows = direct child
  // divs, cells = each row's child divs) into a proper md2jcr block TABLE.
  // Raw block-divs flatten to <p> in html2md; only createTable output survives.
  ...
  // section-metadata block (classes_customClass text field) — md2jcr-safe.
  ...
  // =====================================================================
  // 1. BRAND-EXPLORER — VERBATIM copy of homepage index.plain.html lines 3-80.
  // =====================================================================
...
Ulcerative Colitis|https://www.humira.com/hcp/uc|moderate</div>
  ...
  // =====================================================================
  // 2. HERO no-padding — 8-row table (modeled on dosing-lab-monitoring hero).
  //    row0 = desktop bg picture; row4 = textRow (headline IMAGE + H2 eyebrow +
  //    CTAs — textRow matches because it carries headings/links); row5 = coverage
  //    badge picture (layersRow -> .hero-badge); other rows empty.
  //    ASSUMPTION: headline is an IMAGE (user decision). Mobile headline svg is
  //    provided as a 2nd <picture> in the same headline cell so hero.js
  //    mergeMobileImage can swap it; bg desktop image carries no mobile variant.
  // =====================================================================
  const heroHeadline = picture(`${DAM}/headline-double-arrows.png`, {
    alt: "Control that's fast and shown to last.", width: 1680, height: 296,
  });
  const heroHeadlineMobile = picture(`${DAM}/headline-double-arrows-mobile--2.svg`, {
    alt: "Control that's fast and shown to last.",
  });
  const heroH2 = heading('h2', 'RA patients met <strong>ACR20 at Week 12 or 14</strong> (primary endpoints) and disease control through remission (DAS28-CRP &lt;2.6)‡ at Weeks 12 or 14 and observed up to 5 years.¹,³⁻⁶');
  const heroCta1 = para('');
  ...
  // headline images authored as separate <p> so the hero picture-merge sees two
  // <picture> elements in the text cell; H2 + CTAs follow.
  const heroText = div(
    para(''),
  );
  ...
  // =====================================================================
  // 3. FOOTNOTES / ABBREVIATIONS (default content).
  // =====================================================================
  ...
  // =====================================================================
  // 4. INDICATION callout — text-container boxed-warning (5-row).
  //    Verbatim RA-specific indication + Limitations of Use.
  // =====================================================================
...
<p><strong>Limitations of Use:</strong> RINVOQ is not recommended for use in combination with other Janus kinase (JAK) inhibitors, biologic disease-modifying antirheumatic drugs (bDMARDs), or with potent immunosuppressants such as azathioprine and cyclosporine.</p></div></div>`);
  ...
  // =====================================================================
  // 5. BENEFIT CARDS (cards-grid cards-grid-cta-card) + ACCESS SUPPORT default content.
  //    Card cell shape (homepage): [empty, icon/media, title <p><strong>, body, empty, empty].
  // =====================================================================
  const benefitCards = [
    {
      title: 'Rapid Relief¹,⁷,⁸',
...
<li>27 clinical trials across 9 indications, including an older population (mean age 71) in GCA</li>
  ...
  // 5b. Exceptional Patient and Access Support — its own section so it can be
  // styled as a standalone white card (live: abbv-rinra-homepage-exc-pat-acc).
  out.append(heading('h3', 'Exceptional Patient and Access Support¹¹'));
  out.append(fromHTML(`<ul>
<li>~99% preferred combined National commercial and Medicare Part D formulary coverage under the pharmacy benefit as of November 2025 in RA§||</li>
...
<li>Get patients started on RINVOQ Complete by downloading the enrollment form</li>
  ...
  // 5c. footnotes ‡ § || + abbreviations — rendered below the card (live: outside).
  ...
  out.append(para('§RINVOQ is on a preferred tier or otherwise has preferred status on the plan’s formulary.'));
  ...
  // =====================================================================
  // 7a. PRIMARY ENDPOINTS — H2 heading. On live this H2 sits on the WHITE
  //     page ABOVE the grey "glacier" card, so it is its own section.
  // =====================================================================
  out.append(heading('h2', 'RINVOQ (upadacitinib) Met Its Primary Endpoints in 4 Trials Across Patient Populations and Comparators¹,³,⁷,⁸,¹²'));
  out.append(sectionMeta('ra-endpoints-heading'));

  out.append(hr());

  // =====================================================================
  // 7b. PRIMARY ENDPOINTS glacier card (grey) — chart header + chart image +
  //     chart footnote + GOLD comparator box + clinical-decisions + HUMIRA
  //     links + 4 study cards + ACR link + final abbreviations footnote.
  // =====================================================================
  ...
  out.append(chartP);
  // chart footnote (Unicode sups)
  out.append(para('*NRI-MI (non-responder imputation incorporating multiple imputation). †P≤0.001 vs placebo. ‡P&lt;0.001 vs HUMIRA. §P=0.0001 vs methotrexate.'));
  // comparator disclaimer — GOLD box on live (ra-gold-container). Verbatim live
  // copy (5 short lines joined with <br>) inside a text-container block.
  ...
  // 4 study cards (cards-grid-cta-card). "See Study Details" links to study-design
  // chart image (USER DECISION: modals deferred).
  const studyCards = [
    {
  ...
  // ACR criteria link (modal deferred -> placeholder href) + abbreviations.
  const acrLink = para('');
  ...
  // =====================================================================
  // 8. ISI text-container boxed-warning (full INDICATION) — VERBATIM homepage lines 190-220.
  // =====================================================================
...
<p><strong>Limitations of Use:</strong> RINVOQ/RINVOQ LQ is not recommended for use in combination with other JAK inhibitors, bDMARDs, or with potent immunosuppressants such as azathioprine and cyclosporine.</p></div></div>`);
  ...
  // =====================================================================
  // 9. ISI text-container legal (full ISI) — VERBATIM homepage lines 226-274.
  // =====================================================================
...
<p><strong>Neutropenia:</strong> Treatment with RINVOQ was associated with an increased incidence of neutropenia (ANC &lt;1000 cells/mm³).</p>
<p><strong>Lymphopenia:</strong> ALC &lt;500 cells/mm³ were reported in RINVOQ-treated patients.</p>
...
<p>Please see full <a href="https://www.rxabbvie.com/pdf/rinvoq_pi.pdf">Prescribing Information</a>.</p></div></div></div>
  ...
  // Re-emit as a proper md2jcr block table. The hand-written block's first row
  // is the variant-name row ("legal") which blockFromDiv would duplicate against
  // the block-name cell — so drop it and keep rows 2..5 (-, none, -, richtext).
  {
    const legalRoot = isiLegal.firstElementChild;
    legalRoot.removeChild(legalRoot.firstElementChild); // drop the "legal" name row
  ...
  // =====================================================================
  // 10. text-container references — full 13-reference ordered list (analysis.md).
  // =====================================================================
...
<li>AbbVie Inc. Protocol M23-700. SELECT-SWITCH. EUCT number 2022-502578-18-00. Accessed November 21, 2025.</li>
  ...
  // =====================================================================
  // 11. SAFETY-BAR split — VERBATIM homepage lines 295-387, id -> ra-safety-bar.
  // =====================================================================
  const safetyBar = fromHTML(`<div class="safety-bar split">
    ...
    <div>
        <div><p><strong>IMPORTANT SAFETY INFORMATION &amp; INDICATIONS¹</strong></p>
<h3>INDICATIONS¹</h3>
<p><strong>RINVOQ is indicated for the treatment of:</strong></p>
<ul>
<li><strong>Moderately to severely active rheumatoid arthritis (RA)</strong> in adults who have had an inadequate response or intolerance to one or more tumor necrosis factor (TNF) blockers.</li>
...
<h4>LABORATORY ABNORMALITIES</h4>
<p><strong>Neutropenia</strong><br>Treatment with RINVOQ was associated with an increased incidence of neutropenia (absolute neutrophil count [ANC] &lt;1000 cells/mm³). Treatment with RINVOQ is not recommended in patients with an ANC &lt;1000 cells/mm³. Evaluate neutrophil counts at baseline and thereafter according to routine patient management.</p>
<p><strong>Lymphopenia</strong><br>Absolute lymphocyte counts (ALC) &lt;500 cells/mm³ were reported in RINVOQ-treated patients. Treatment with RINVOQ is not recommended in patients with an ALC &lt;500 cells/mm³. Evaluate at baseline and thereafter according to routine patient management.</p>
...
<p>Please see full <a href="https://www.rxabbvie.com/pdf/rinvoq_pi.pdf">Prescribing Information</a>.</p></div>
  ...
  // Replace all of body's content with the composed output.
